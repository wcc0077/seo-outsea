...
export default async function ContactPage({ params }: ContactPageProps) {
  ...
  // Progressive migration: try Strapi page first
  const page = await getPageBySlug('contact', locale).catch(() => null);
  if (page) {
  ...
  const phone = globalData?.contactInfo?.phone || '4000-56-5516';
  const email = globalData?.contactInfo?.email || 'sales@fn-tech.com';
